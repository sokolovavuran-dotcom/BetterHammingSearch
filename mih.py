...
def run_mih(dataset_dir: str, prefix: str, radius: int, m: int) -> None:
    ...
    t0 = time.perf_counter()
    results = [
        mih_query(tables, seg, base_codes, query_codes[i], radius, masks)
        for i in range(Q)
    ]
...
def main() -> None:
    ...
    run_mih(dataset_dir, prefix, args.radius, args.segments)
